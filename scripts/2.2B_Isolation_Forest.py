...
Method B of the Layer 2 anomaly detection ensemble — Isolation Forest.
...
Design:
    Feature matrix  29 raw KPI columns from master_dataset  (0 NaN — no imputation needed)
    Scaler          StandardScaler  (fit on train set only — no data leakage)
...
Output:
    ...
    method_b_score        negated decision_function score  — higher = more anomalous
...
OUTPUT_CSV  = DATA / "method_b_results.csv"
...
def get_feature_cols(df: pd.DataFrame) -> list[str]:
    """29 raw KPI columns — original master_dataset values, no engineered features."""
    return [
...
def run_isolation_forest(df: pd.DataFrame,
    ...
    print(f"  Score rows : {len(df)}  "
          f"(full dataset — in-sample detection)")
...
def main() -> None:
    ...
    print(f"  Feature columns: {len(feature_cols)}  (0 NaN — no imputation required)")
    print()
