...
INPUT_CSV  = os.path.join(DATA_DIR, "intelligence", "intelligence_results.csv")
OUTPUT_CSV = os.path.join(DATA_DIR, "communication", "alert_payloads.csv")
...
check(
    "T06  INVESTIGATE rows | audience='Operations, Analyst' | urgency='Daily'",
    len(inv) > 0
...
check("T12  SQLite alert_payloads row count matches", db_n == len(df), f"got {db_n}")
if failures:
    sys.exit(1)
