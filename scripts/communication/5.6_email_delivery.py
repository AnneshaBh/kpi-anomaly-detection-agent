"""
Layer 5, Step 5.6 â€” Email Delivery
...
Routing
  ESCALATE    â†’ sent immediately  (up to MAX_EMAILS_ESCALATE)
  INVESTIGATE â†’ sent in batch     (up to MAX_EMAILS_INVESTIGATE)
  MONITOR     â†’ sent in batch     (up to MAX_EMAILS_MONITOR)
  SUPPRESSED  â†’ skipped entirely (external factors, no action needed)

SMTP config (.env)
  SMTP_HOST             â€” mail server host       (default: smtp.gmail.com)
  SMTP_PORT             â€” mail server port       (default: 587)
  SMTP_USER             â€” sender Gmail address
  SMTP_PASSWORD         â€” 16-char Gmail App Password (spaces allowed)
  ALERT_RECIPIENTS      â€” comma-separated recipient list
  MAX_EMAILS_ESCALATE   â€” cap on ESCALATE emails per run
  MAX_EMAILS_INVESTIGATEâ€” cap on INVESTIGATE emails per run
  MAX_EMAILS_MONITOR    â€” cap on MONITOR emails per run
...
INPUT_CSV = os.path.join(DATA_DIR, "communication", "communication_results.csv")
...
# Recipients â€” comma-separated in .env, e.g. a@x.com,b@x.com
ALERT_RECIPIENTS = [
...
# â”€â”€ Validate config â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
missing = []
...
# â”€â”€ Apply caps and build send queue â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
print(f"  {'SUPPRESSED':<12}  {n_suppressed:>3} skipped  (external factors â€” no action needed)")
print()
...
try:
    ...
    print(f"  ERROR: SMTP connection failed â€” {exc}")
    sys.exit(1)
print()
...
for _, row in send_df.iterrows():
    ...
    try:
        ...
        print(f"  FAILED  [{flag:<11}]  #{rank:<4}  {anomaly_id}  â€” {exc}")
...
print("=" * 68)
print("LAYER 5 STEP 5.6 â€” EMAIL DELIVERY SUMMARY")
print("=" * 68)
print()
...
print(f"  {'SUPPRESSED':<14}  {'â€“':>6}  (skipped)")
print()
