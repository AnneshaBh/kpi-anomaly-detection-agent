...
Graph direction: edges run effect → cause.
  Reading "X → Y": "Y is a suspected upstream driver of X."
...
Traversal logic:
    ...
    Tier 2 / 3 (return_rate, n_stockouts, bounce_rate, etc.):
        ...
        No further traversal — the KPI itself is the suspected driver.
        Records which Tier 1 KPIs this anomaly transitively feeds into.

Inputs:
    data/anomaly_results.csv           181 confirmed KPI-day anomalies
    data/processed_kpi_features.csv    731 × 183 — z-scores per date per KPI
...
OUT_CSV     = DATA / "rca_graph_results.csv"
...
def build_dependency_graph() -> nx.DiGraph:
    ...
    Edges: effect → cause  (the direction of investigation when an anomaly fires).
    ...
    (multiple parents) — this is valid in a DAG, no cycles are introduced.
    """
    G = nx.DiGraph()
    ...
    # conversion_rate tree — reachable as child of n_orders AND as Tier 1 root
    # bounce_rate as cause of conversion_rate drop (high bounce → low conversion)
    ...
    assert nx.is_directed_acyclic_graph(G), "Dependency graph contains a cycle — check edge definitions."
...
def traverse(
    ...
    """
    DFS from root_kpi following effect → cause edges.
...
def main() -> None:
    ...
    for _, row in anomalies.iterrows():
        ...
        # ── Co-anomalous KPIs: other monitored KPIs at |z| >= threshold ──
        co_anomalous = [
            other_kpi
        ...
        # ── Tier 2 / 3: no traversal — KPI is already the driver ──
        else:
            path            = [kpi]
