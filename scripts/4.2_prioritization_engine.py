...
INPUT_CSV  = os.path.join(DATA_DIR, "impact_results.csv")
OUTPUT_CSV = os.path.join(DATA_DIR, "priority_results.csv")
...
assert df.shape == (181, 51), f"Expected (181, 51), got {df.shape}"
...
assert out.shape == (181, 59), f"Expected (181, 59), got {out.shape}"
assert out["priority_score"].between(0, 1).all(), "priority_score out of [0, 1]"
assert out["priority_rank"].nunique() == 181, "priority_rank contains duplicates"
