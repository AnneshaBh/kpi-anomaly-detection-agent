...
AP_CSV      = os.path.join(DATA_DIR, "alert_payloads.csv")
DL_CSV      = os.path.join(DATA_DIR, "delivery_log.csv")
OUTPUT_CSV  = os.path.join(DATA_DIR, "communication_results.csv")
...
check(
    "T05  All 15 ESCALATE rows -> delivery_status=SENT",
    len(esc) == 15 and (esc["delivery_status"] == "SENT").all(),
...
check(
    "T08  message_id: 181 unique values",
    df["message_id"].nunique() == 181,
...
check(
    "T09  ANO-20241129-REV: priority_rank=1, delivery_status=SENT, revenue_at_risk<0",
    len(bf) == 1
...
rank_ok  = df["priority_rank"].nunique() == 181 and set(df["priority_rank"]) == set(range(1, 182))
score_ok = df["priority_score"].between(0, 1).all() and df["priority_score"].notna().all()
check(
    "T10  Layer 4 preserved: priority_rank 1-181 unique, priority_score in [0,1]",
...
check(
    "T11  Layer 4 preserved: 101 LLM-enhanced rows, all with non-empty immediate_action",
    len(llm_rows) == 101 and empty_imm == 0,
...
EXPECTED_TABLES = {
    ...
    "alert_payloads"        :   181,
    "delivery_log"          :   181,
    "communication_results" :   181,   # will be written below
}
conn = sqlite3.connect(DB_PATH)
db_tables = [r[0] for r in conn.execute(
    "SELECT name FROM sqlite_master WHERE type='table'"
).fetchall()]
if "communication_results" in db_tables:
    db_n = conn.execute("SELECT COUNT(*) FROM communication_results").fetchone()[0]
    check("T12  SQLite communication_results = 181 rows", db_n == 181, f"got {db_n}")
else:
    print("  INFO  T12  communication_results not yet in SQLite (will be written below)")
conn.close()
...
for status, n in df["delivery_status"].value_counts().items():
    print(f"  {status:<12}  {n:>3}  ({n/181*100:>5.1f}%)")
