"""
Layer 6 â€” Step 6.1: Agent Tool Definitions

Nine Python functions that the orchestrator agent (Step 6.2) calls via Anthropic tool_use.
Each tool queries the pre-computed Layer 1â€“5 CSV outputs in data/ rather than re-running
detection or causal inference pipelines from scratch.

Also exports:
  TOOL_DEFINITIONS  â€” list of Anthropic tool schemas (passed to the Claude API)
  dispatch_tool()   â€” routes a tool_name + tool_input dict to the correct function
"""
...
def fetch_kpis(date: str) -> dict:
    ...
    df = _read("processed/master_dataset.csv")
    row = df[df["date"] == date]
    if row.empty:
...
def prioritize(date: str) -> dict:
    ...
    df = _read("intelligence/priority_results.csv")
...
def send_alert(anomaly_id: str) -> dict:
    ...
    df = _read("communication/delivery_log.csv")
    row = df[df["anomaly_id"] == anomaly_id]
    if row.empty:
...
# ---------------------------------------------------------------------------
# Tool 8 â€” generate_executive_summary
# ---------------------------------------------------------------------------
...
def generate_executive_summary(date: str, summary_text: str) -> dict:
    ...
    path = alerts_dir / f"executive_summary_{date}.txt"
...
# ---------------------------------------------------------------------------
# Tool 9 â€” update_dashboard_dataset
# ---------------------------------------------------------------------------
...
def update_dashboard_dataset(date: str) -> dict:
    ...
    """
    df = _read("communication/communication_results.csv")
    rows = df[df["date"] == date]
    if rows.empty:
        return {"error": f"No communication_results rows for date {date}", "rows_written": 0}

    out_path = DATA_DIR / "agent/agent_results.csv"
    if out_path.exists():
...
TOOL_DEFINITIONS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "suspected driver KPI, CausalImpact effect estimate, DoWhy ATE, root_cause_confidence (0â€“1), "
    ...
    },
    {
        ...
        "description": (
            ...
            "Priority factors: revenue_impact Ã— KPI tier Ã— causal confidence Ã— recoverability âˆ’ external_driver_penalty. "
    ...
    },
    {
        ...
        "description": (
            ...
            "HIGH severity â†’ Slack + Email (SENT). "
            "MEDIUM severity â†’ Email (QUEUED). "
            "LOW severity â†’ Digest (SCHEDULED). "
            "Externally driven + suppressed â†’ SUPPRESSED. "
...
if __name__ == "__main__":
    ...
    result = generate_executive_summary(
        TEST_DATE,
        f"TEST SUMMARY for {TEST_DATE}: pipeline smoke test â€” {len(anomalies)} anomalies detected."
    )
