...
def _anomaly_df() -> pd.DataFrame:
    return pd.read_csv(DATA_DIR / "anomaly_results.csv")
...
def _scheduled_job() -> None:
    ...
    most recent dataset date — supports both production and demo modes.
    """
    ...
    if today_str in df["date"].values:
        ...
        print(
            ...
            f"Today ({today_str}) not in dataset — using latest: {run_date}"
        )
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            "Layer 6 Agent Runner — runs the KPI Anomaly Detection pipeline "
