...
Six features are added per KPI  (column naming: {kpi}_{feature}):
    rolling_mean   7-day rolling mean             â€” smooths daily noise
    rolling_std    7-day rolling std              â€” dynamic threshold baseline
    z_score        (kpi - rolling_mean) / rolling_std  â€” standardised deviation
    wow_change     (today - 7d ago)  / |7d ago|   â€” week-over-week % shift
    mom_change     (today - 30d ago) / |30d ago|  â€” month-over-month % shift
    lag_1          prior-day value                â€” short-term momentum input

KPI tiers (from architecture):
    Tier 1 â€” alert immediately:   total_revenue_usd, n_orders, avg_roas, conversion_rate
    Tier 2 â€” alert within 1 hour: return_rate, n_stockouts, avg_order_value_usd, bounce_rate
    Tier 3 â€” daily digest:        total_clicks, sessions, inventory_health, avg_discount_pct
...
OUTPUT_CSV  = DATA / "processed/processed_kpi_features.csv"
OUTPUT_DB   = DATA / "db/kpi_anomaly_detection.db"
DB_TABLE    = "processed_kpis"

ROLLING_WINDOW = 7    # days â€” rolling mean / std window
WOW_LAG        = 7    # days â€” week-over-week comparison
MOM_LAG        = 30   # days â€” month-over-month comparison

# Non-KPI columns â€” excluded from feature engineering
NON_KPI_COLS = {
    "date",
    # Exogenous control variables â€” model inputs, not detection targets
...
def engineer_features(df: pd.DataFrame, kpi_cols: list[str]) -> pd.DataFrame:
    ...
    for kpi in kpi_cols:
        ...
        # â”€â”€ Rolling statistics â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        roll_mean = series.rolling(ROLLING_WINDOW, min_periods=1).mean()
        roll_std  = series.rolling(ROLLING_WINDOW, min_periods=2).std()

        # â”€â”€ Z-score: (value âˆ’ mean) / std â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        # Replace zero std with NaN so result is NaN rather than Â±Inf
        safe_std  = roll_std.replace(0, np.nan)
        z_score   = (series - roll_mean) / safe_std

        # â”€â”€ WoW % change: (today âˆ’ 7d ago) / |7d ago| â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        base_wow  = series.shift(WOW_LAG)
        wow       = (series - base_wow) / base_wow.replace(0, np.nan).abs()

        # â”€â”€ MoM % change: (today âˆ’ 30d ago) / |30d ago| â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        base_mom  = series.shift(MOM_LAG)
        mom       = (series - base_mom) / base_mom.replace(0, np.nan).abs()

        # â”€â”€ 1-day lag â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # Single concat â€” avoids fragmentation from per-column inserts
    features_df = pd.DataFrame(feature_dict, index=df.index)
    return pd.concat([df, features_df], axis=1)


# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Step 4 â€” Write outputs
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
