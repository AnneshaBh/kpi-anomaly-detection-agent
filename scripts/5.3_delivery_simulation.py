...
INPUT_CSV    = os.path.join(DATA_DIR, "alert_payloads.csv")
OUTPUT_CSV   = os.path.join(DATA_DIR, "delivery_log.csv")
...
check(
    "T04  INVESTIGATE: 86 rows, all QUEUED",
    len(inv) == 86 and (inv["delivery_status"] == "QUEUED").all(),
...
check(
    "T05  MONITOR: 74 rows, all SCHEDULED",
    len(mon) == 74 and (mon["delivery_status"] == "SCHEDULED").all(),
...
check(
    "T06  SUPPRESSED: 6 rows, all SUPPRESSED status",
    len(sup) == 6 and (sup["delivery_status"] == "SUPPRESSED").all(),
...
check(
    "T08  message_id: 0 nulls and 181 unique values",
    null_msg == 0 and uniq_msg == 181,
...
check("SQLite delivery_log = 181 rows", db_n == 181, f"got {db_n}")
if failures:
    sys.exit(1)
