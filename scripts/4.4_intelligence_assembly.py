...
INPUT_CSV  = os.path.join(DATA_DIR, "recommendations.csv")
OUTPUT_CSV = os.path.join(DATA_DIR, "intelligence_results.csv")
...
check(
    "T03  priority_rank unique integers 1-181",
    df["priority_rank"].nunique() == 181 and set(df["priority_rank"]) == set(range(1, 182)),
...
check(
    "T04  All 15 ESCALATE anomalies -> priority_band HIGH",
    len(esc) == 15 and (esc["priority_band"] == "HIGH").all(),
...
check(
    "T05  All 6 SUPPRESSED anomalies: llm_enhanced=False, escalation_suppressed=True",
    len(sup) == 6 and (~sup["llm_enhanced"]).all() and sup["escalation_suppressed"].all(),
...
check(
    "T06  All 74 MONITOR anomalies: llm_enhanced=False",
    len(mon) == 74 and (~mon["llm_enhanced"]).all(),
...
check(
    "T07  Black Friday 2024-11-29 revenue: rank=1, priority_band=HIGH, revenue_at_risk<0",
    len(bf) == 1
    and int(bf.iloc[0]["priority_rank"]) == 1
...
check(
    "T10  All 101 LLM-enhanced rows have non-empty immediate/short-term/preventive",
    len(llm_rows) == 101 and empty_imm == 0 and empty_st == 0 and empty_pr == 0,
...
if "intelligence_results" in tables:
    db_n = conn.execute("SELECT COUNT(*) FROM intelligence_results").fetchone()[0]
    check("T12  SQLite intelligence_results = 181 rows", db_n == 181, f"got {db_n}")
else:
    # Table doesn't exist yet — will be created below; mark as deferred
    print("  INFO  T12  intelligence_results table not yet in SQLite (will be written below)")
conn.close()
...
check("T12  SQLite intelligence_results = 181 rows", db_n == 181, f"got {db_n}")
if "T12  SQLite intelligence_results = 181 rows" in failures:
    sys.exit(1)
...
for band, n in df["priority_band"].value_counts().reindex(["HIGH","MEDIUM","LOW"]).items():
    print(f"  {band:<8}  {n:>3}  ({n/181*100:>5.1f}%)")
